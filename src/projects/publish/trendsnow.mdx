...
<Grid lgrows={2} lgcolumns={2} mdrows={3} mdcolumns={1} className="rows-auto cols-60-40 no-figure-margins no-row-gaps">
  ...
  <div className="gridarea-1">
    ...
    We needed to replace an existing analytics product which was outdated and lacked many features needed to compete with competitors.
...
<Territories id="territories">
    ...
    The MVP I designed had all the functionality but no sizzle. I evolved it to an interactive map
    visualization for desktop and mobile.
...
<Search id="search">
    ...
    We started with a traditional lookup search in a dropdown list but there's only so much you can show before the user is overwhelmed. So we expanded search to the full window giving us room to better categorize and highlight results.
...
<OnTheMove id="sotm">
    ...
    There are various stages of growth so we needed a styling system to handle varying messaging and emphasis high value insights.
